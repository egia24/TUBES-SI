    ...
    <div class="sidebar-scroll">
        <nav>
            <ul class="nav">
                ...
                <li><a href="/Transaksi_pelanggan" class="active"><i class="lnr lnr-cart"></i> <span>Transaksi Pelanggan</span></a></li>
            ...
            <div class="row">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">TRANSAKSI PELANGGAN</h3><br>
                        <button> <a href="/iTransaksi_pelanggan" type="button" class="btn btn-success">Tambah Data</a></button>
                        @if (count($data) > 0)
                            <button> <a href="/exportTransaksi_pelanggan/{{ $data[0]->id }}" class="btn btn-success">Download PDF</a></button>
                        @endif
                    </div>
                    <div class="panel-body">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th>{{$transaksi_pelanggans->id_transaksi_pelanggan}}</th>
                                    <th>{{$transaksi_pelanggans->pelanggan->id_pelanggan}}</th>
                                    <th>{{$transaksi_pelanggans->barang->id_barang}}</th>
                                    <th>{{$transaksi_pelanggans->jenis_transaksi->id_jenis_transaksi}}</th>
                                    <th>{{$transaksi_pelanggans->jumlah_barang}}</th>
                                    <th>{{$transaksi_pelanggans->tanggal_transaksi_pelanggan}}</th>
                                    <th>{{$transaksi_pelanggans->total_harga_pelanggan}}</th>
                                    <td>
                                        <a href="/editTransaksi_pelanggan/{{ $transaksi_pelanggans->id }}"
                                             data-mdc-auto-init="MDCRipple">
                                            Edit
                                        </a>
                                        <a href="/delTransaksi_pelanggan/{{ $transaksi_pelanggans->id }}"
                                             data-mdc-auto-init="MDCRipple">
                                            Delete
                                        </a>
                                    </td>
